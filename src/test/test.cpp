...
TEST(SORT_TEST_WEIGTH, sorting_ten_numbers) {
    int number = 10;
    auto *arr_to_sort = (product_struct *) (malloc(
            number * sizeof(product_struct)));
...
TEST(TEST_SPLIT_STRUCT_BY_WEIGTH, spliting_three_structure) {
    ...
    auto *waylbill_list = (product_struct *) malloc(
            amount_of_list * sizeof(product_struct));
    EXPECT_NE(waylbill_list, nullptr);

    product_struct waylbill_first_product = {10, 10, 10.00, 10.00, 100.00,
                                             100.00};
    product_struct waylbill_second_product = {20, 20, 20.00, 20.00, 400.00,
                                              400.00};
    product_struct waylbill_third_product = {30, 10, 20.00, 20.00, 200.00,
                                             200.00};
